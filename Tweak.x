...
#pragma mark - Hooks
%hook MTContext
- (void)reportTransportSchemeSuccessForDatacenterId:(NSInteger)datacenterId transportScheme:(id)scheme {
    %orig;
    TGAdoptContext(self, @"success");
    gTransportSuccessAbs = CFAbsoluteTimeGetCurrent();
    TGOnSchemeSuccess();
}
- (void)reportTransportSchemeFailureForDatacenterId:(NSInteger)datacenterId transportScheme:(id)scheme {
    %orig;
...
%hook MTTransport
- (instancetype)initWithDelegate:(id)delegate context:(id)context datacenterId:(NSInteger)datacenterId
                         schemes:(id)schemes proxySettings:(id)proxySettings
              usageCalculationInfo:(id)usageCalculationInfo getLogPrefix:(id)getLogPrefix {
    self = %orig;
    if (self && context) TGAdoptContext(context, @"transport.init");
    return self;
}
%end

// Neutralize the stale checkmark/status in Telegram's proxy list when auto-rotating.
%hook UITableViewCell
- (void)layoutSubviews {
    %orig;
    TGNeutralizeProxyCellIfNeeded(self);
}
- (void)setAccessoryType:(UITableViewCellAccessoryType)accessoryType {
    if (gEnabled && accessoryType == UITableViewCellAccessoryCheckmark) {
        NSMutableArray<UILabel *> *labels = [NSMutableArray array];
        TGCollectLabels(self.contentView ?: (UIView *)self, labels);
        for (UILabel *l in labels) {
            NSString *t = l.text ?: l.attributedText.string;
            if (TGStringMatchesKnownProxyHost(t)) { accessoryType = UITableViewCellAccessoryNone; break; }
        }
    }
    %orig(accessoryType);
}
...
%ctor {
    @autoreleasepool {
        ...
        %init;
        gProxies = [NSMutableArray array];
        TGPrefsApply(nil);
        TGLog(@"=== TGProxyRotation loaded pid=%d ===", getpid());

        // If the external list is enabled, seed it from the on-disk cache right
        // away so proxies are usable immediately at launch, then refresh online.
        if (gTelemtSearch) {
            TGLoadOnlineCache();
            TGFetchOnlineProxies();
        }

        // Mark startup pending: the first watchdog tick will apply the saved
        // proxy (absorbing the phantom first tick) instead of rotating forward.
        gStartupPending = gEnabled;

        if (gEnabled) TGWatchStart();

        CFNotificationCenterAddObserver(CFNotificationCenterGetDarwinNotifyCenter(),
            NULL, TGOnDarwinNotify, CFSTR("com.ratush.tgproxyrotation.changed"), NULL,
            CFNotificationSuspensionBehaviorCoalesce);

        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(2.5*NSEC_PER_SEC)),
                       dispatch_get_main_queue(), ^{
            TGInstallGestures();
            // Auto-show the minimized shield FAB on launch so the user does not
            // need to long-press to reveal it. Show the panel silently (no
            // fade-in animation) then immediately minimize to the FAB.
            gPanel = [TGPanelController shared];
            [gPanel show];
            gPanel.card.alpha = 0;
            gPanel.card.hidden = YES;
            [gPanel minimize];
        });

        // Re-read the proxy list after Telegram fully loads its postbox DB,
        // since the early read at launch may only see 1 proxy (DB not ready).
        // The saved proxy is applied by the startup tick in the watchdog.
        for (int delay = 5; delay <= 15; delay += 5) {
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(delay * NSEC_PER_SEC)),
                           dispatch_get_main_queue(), ^{
                NSArray *before; @synchronized(gProxies) { before = [gProxies copy]; }
                TGPrefsApply(nil);
                NSArray *after; @synchronized(gProxies) { after = [gProxies copy]; }
                if (after.count != before.count) {
                    TGLog(@"late proxy reload: %lu -> %lu proxies",
                          (unsigned long)before.count, (unsigned long)after.count);
                    TGUpdatePanel();
                }
            });
        }
